...
class RiotAPIService {
  ...
  async getLeaderboard(actId, region = 'eu', size = 200, startIndex = 0) {
    try {
      const platformUrl = this.platformUrls[region] || this.platformUrls.eu;
      const url = `${platformUrl}/val/ranked/v1/leaderboards/by-act/${actId}?size=${size}&startIndex=${startIndex}`;
      return await this.makeRequest(url, `leaderboard:${region}:${actId}:${size}:${startIndex}`);
    } catch (error) {
  ...
  // Get current competitive act ID
  async getCurrentAct(region = 'eu') {
    try {
      const platformUrl = this.platformUrls[region] || this.platformUrls.eu;
      const url = `${platformUrl}/val/content/v1/contents`;
      const content = await this.makeRequest(url, `acts:${region}`);
      
      // Find current competitive act
      const acts = content.acts || [];
      const currentAct = acts.find(act => act.isActive && act.type === 'act');
      
      return currentAct ? currentAct.id : null;
    } catch (error) {
      console.error('Error getting current act:', error);
      return null;
    }
  }

  // Get leaderboard with current act
  async getLeaderboardCurrent(region = 'eu', size = 200, startIndex = 0) {
    try {
      const actId = await this.getCurrentAct(region);
      if (!actId) {
        throw new Error('Could not find current competitive act');
      }
      
      return await this.getLeaderboard(actId, region, size, startIndex);
    } catch (error) {
